...
EXPERIMENT_DESCRIPTION = 'MoCo Spatial Representations FastFCN Downstream'
EXPERIMENT_TAG = 'moco_spatial_representations_fastfcn_downstream'


def main():
    MoCoDownStreamSpatialRepresentationsFastFCNExperiment().run()


class MoCoDownStreamSpatialRepresentationsFastFCNExperiment(rs.framework.Experiment):
    ...
    def create_argument_parser(self, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        parser.add_argument(
            ...
            type=float,
            default=1e-4,
            help='Weight decay for convolution weights in backbone'
        )
        parser.add_argument(
            '--backbone',
            type=str,
        ...
        parser.add_argument('--fcn-weight-decay', type=float, default=1e-4, help='Weight decay for convolution weights')
        ...
        parser.add_argument(
            ...
            type=bool,
            default=True,
            help='Whether or not backbone should be fine-tuned'
        )
        parser.add_argument(
            '--backbone-checkpoint-file',
            type=str,
            required=True,
            help='filepath to checkpoint file of pretrained MoCo backbone'
        )
    ...
    def build_parameter_dict(self, args: argparse.Namespace) -> typing.Dict[str, typing.Any]:
        return {
            ...
            'backbone_kernel_initializer': 'he_uniform',
            'backbone_dense_initializer': 'he_uniform',
            'jpu_features': 512,  # FIXME: We could decrease those since we have less classes.
            'fcn_weight_decay': args.fcn_weight_decay,
            ...
            'fcn_kernel_initializer': 'he_normal',  # FIXME: This might not necessarily be the best choice
            'fcn_dense_initializer': 'he_uniform',  # Only for the dense weights in the Encoder head
            ...
            'backbone_checkpoint_file': args.backbone_checkpoint_file
        }
    ...
    def fit(self) -> typing.Any:
        ...
        model = rs.models.fastfcn.FastFCN(
            ...
            self.parameters['fcn_kernel_initializer'],
            self.parameters['fcn_dense_initializer'],
        ...
        self.log.info('Load backbone weights')
        if not os.path.exists(self.parameters['backbone_checkpoint_file']):
            raise FileExistsError('Checkpoint file "{}" does not exist'.format(self.parameters['backbone_checkpoint_file']))
        backbone.load_weights(self.parameters['backbone_checkpoint_file'])
        backbone.trainable = self.parameters['fine_tune_backbone']
    ...
    def _construct_backbone(self, name: str) -> tf.keras.Model:
        ...
        kwargs = {
            'kernel_regularizer': tf.keras.regularizers.L1L2(l2=self.parameters['backbone_weight_decay']),
            'kernel_initializer': self.parameters['backbone_kernel_initializer'],
    ...
    def _augment_sample(self, image: tf.Tensor, mask: tf.Tensor) -> typing.Tuple[tf.Tensor, tf.Tensor]:
        ...
        scaled_image = tf.image.resize(blurred_image, scaled_size,
                                       method=self.parameters['augmentation_interpolation'])
    ...
    def _build_optimizer(self, steps_per_epoch: int) -> tfa.optimizers.SGDW:
        ...
        weight_decay_factor = self.parameters['fcn_weight_decay'] / self.parameters['initial_learning_rate']
